...
With a smaller dataset, the `load()` commands works fastest. However, this quickly changes as data gets larger. In general, saving data in one of these three formats is the simplest and should suffice for most use cases you'll have.^[A topic that is beyond the scope of this workshop is loading and working with JSON data, which you'll typically need to do if you're downloading data directly from an API. The R package **jsonlite** contains a variety of very useful tools for working with JSON data.]
